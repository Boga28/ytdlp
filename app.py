...
def get_videos(url, extra_params):
    ...
    '''
    retrieved_cookie_content_from_env = os.environ.get('YOUTUBE_COOKIES_CONTENT4')
    if retrieved_cookie_content_from_env:
        current_app.logger.info(f"DEBUG (in get_videos): YOUTUBE_COOKIES_CONTENT4 IS SET. Length: {len(retrieved_cookie_content_from_env)}. First 100 chars: '{retrieved_cookie_content_from_env[:100]}'")
    else:
        current_app.logger.warning("DEBUG (in get_videos): YOUTUBE_COOKIES_CONTENT4 IS NOT SET or is empty in the environment.")
    ...
    if 'cookies' not in ydl_params:
        ...
        if configured_cookie_file:
            if os.path.exists(configured_cookie_file):
             ...
             current_app.logger.info("No YOUTUBE_COOKIES_CONTENT4 env var and no COOKIES_FILE_PATH configured. Proceeding without explicit cookies.")
